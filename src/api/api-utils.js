...
class ApiUtils {
  ...
  static champions = {"zh_TW": [], "en_US": [], "vi_VN": []}; // 根據語言快取
  static lastFetchedChampions = {"zh_TW": null, "en_US": null, "vi_VN": null}; // 存儲上次獲取數據的時間戳
    ...
    const cacheChampionsData = async () => {
      try {
        await Promise.all([
          ...
          ApiUtils.getAllChampionsByCache('vi_VN')
        ]);
  ...
  static getCurrentLang() {
    if (i18next.language === 'en') return 'en_US';
    if (i18next.language === 'vi') return 'vi_VN';
    return 'zh_TW';
  }
  ...
  /**
   * 獲取 Lobby 成員的 PUUID 陣列
   * @returns {Promise<string[]>} 成員 of PUUID
   */
  ...
  // --- NEW FEATURES LCU API CALLS ---

  static patchChampSelectAction(actionId, championId) {
    return axios.patch(`${ApiUtils.getApiBaseUrl()}/lol-champ-select/v1/session/actions/${actionId}`, { championId });
  }

  static postChampSelectActionComplete(actionId) {
    return axios.post(`${ApiUtils.getApiBaseUrl()}/lol-champ-select/v1/session/actions/${actionId}/complete`);
  }

  static patchChampSelectSpells(spell1Id, spell2Id) {
    return axios.patch(`${ApiUtils.getApiBaseUrl()}/lol-champ-select/v1/session/my-selection`, {
      spell1Id: Number(spell1Id),
      spell2Id: Number(spell2Id)
    });
  }

  static async putRunePage(name, primaryStyleId, subStyleId, selectedPerkIds) {
    try {
      const pagesResponse = await axios.get(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages`);
      const pages = pagesResponse.data;
      const editablePage = pages.find(p => p.isEditable);
      const payload = {
        name: name || "Auto Page",
        primaryStyleId: Number(primaryStyleId),
        subStyleId: Number(subStyleId),
        selectedPerkIds: selectedPerkIds.map(Number),
        current: true
      };
      if (editablePage) {
        await axios.put(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages/${editablePage.id}`, payload);
      } else {
        await axios.post(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages`, payload);
      }
      console.log("Rune page updated successfully");
    } catch (error) {
      console.error("Error setting rune page:", error);
    }
  }

  static putChatAvailability(availability) {
    return axios.put(`${ApiUtils.getApiBaseUrl()}/lol-chat/v1/me`, { availability })
      .then(response => console.log("putChatAvailability success", response.data))
      .catch(error => console.error("putChatAvailability error", error));
  }

  static postSummonerProfile(key, value) {
    return axios.post(`${ApiUtils.getApiBaseUrl()}/lol-summoner/v1/current-summoner/summoner-profile`, { key, value })
      .then(response => console.log("postSummonerProfile success", response.data))
      .catch(error => console.error("postSummonerProfile error", error));
  }

  static postStartMatchmaking() {
    return axios.post(`${ApiUtils.getApiBaseUrl()}/lol-lobby/v2/lobby/matchmaking/search`)
      .then(response => console.log("postStartMatchmaking success", response.data))
      .catch(error => console.error("postStartMatchmaking error", error));
  }
